...
void forward_muon_contributions_macro() {
    ...
    // W
    TFile* infile_W = TFile::Open("mymain11Hist_joined.root", "READ");
    ...
    HF_muon_pt->GetYaxis()->SetTitle("#frac{d#sigma}{dp_{T}} (pb/GeV/c)");
    HF_muon_pt->SetMinimum(0.01);
    HF_muon_pt->SetStats(0);
    HF_muon_pt->SetLineColor(4);
    HF_muon_pt->SetMarkerStyle(24);
    HF_muon_pt->SetMarkerColor(4);
    HF_muon_pt->Draw("SAME");

    W_muon_pt->SetLineColor(3);
    W_muon_pt->SetMarkerStyle(26);
